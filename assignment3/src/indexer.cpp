...
const document & indexer::operator [](string name){
    for(vector<document>::const_iterator docsIt = documents.begin(); docsIt != documents.end(); ++docsIt){
        if(docsIt->name() == name)
            return *docsIt;
    }
}
 ...
 */
const document & operator >>(document & d, indexer & idx){
	...
	string cont = d.get_content(); // string content of document
	int total_1 = 0;
	int total_2 = 0;
...
ostream & operator <<(ostream & os,indexer & idx) {
    ...
    for(map<string, map<string, int> >::const_iterator termIterator = idx.tftd1.begin(); termIterator != idx.tftd1.end(); ++termIterator){
        ...
        for(vector<string>::const_iterator documentNameIt = documentNames.begin(); documentNameIt != documentNames.end(); ++documentNameIt){
            string docName = *documentNameIt;
            int freq;
            if(isnan(docFrequency[docName])){
                freq = 0;
            }else{
                freq = docFrequency[docName];
            }
    ...
    for(map<string, map<string, int> >::const_iterator termIterator = idx.tftd2.begin(); termIterator != idx.tftd2.end(); ++termIterator){
        string term = termIterator->first;
        map<string, int> docFrequency = termIterator->second;
    ...
    for (map<string, map<string, double> >::const_iterator it = idx.wtd.begin(); it != idx.wtd.end(); ++it) {
        ...
        for(vector<string>::const_iterator documentNameIt = documentNames.begin(); documentNameIt != documentNames.end(); ++documentNameIt){
            string docName = *documentNameIt;
            double weight;
            if(isnan(docWeight[docName])){
                weight = 0;
            }else{
                weight = docWeight[docName];
            }
